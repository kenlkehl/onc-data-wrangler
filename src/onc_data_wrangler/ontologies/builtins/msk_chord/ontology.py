...
from .dictionary import MSKChordDictionary
...
class MSKChordOntology(OntologyBase):
    ...
    def __init__(self):
        ...
        # Load the CDM data dictionary from CSV
        self._chord_dictionary = MSKChordDictionary()
        self._chord_dictionary.load()
    ...
    def get_code_resolver(self):
        """Return a GenericCodeResolver built from dictionary allowable values.

        Scans all MSK-CHORD dictionary fields for those with inferred
        allowable values and builds a resolver map so the extraction
        pipeline can validate LLM output against known value sets.
        """
        from ....extraction.code_resolver import GenericCodeResolver

        valid_values_map: Dict[str, Dict[str, str]] = {}
        for field_item in self._chord_dictionary.get_fields_with_allowable_values():
            av = field_item.allowable_values
            if not av:
                continue
            # Parse comma-separated values into a {value: value} dict
            values = [v.strip() for v in av.split(",") if v.strip()]
            if values:
                valid_values_map[field_item.field_id] = {v: v for v in values}

        if not valid_values_map:
            return None

        logger.debug(
            "MSK-CHORD code resolver built with %d fields", len(valid_values_map)
        )
