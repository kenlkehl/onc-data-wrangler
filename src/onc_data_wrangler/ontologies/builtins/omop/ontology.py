...
from .dictionary import OMOPDictionary
...
class OMOPOntology(OntologyBase):
    ...
    def __init__(self):
        """Initialize and load table definitions and OMOP dictionary."""
        self._vocabularies = self._load_vocabularies()
        self._oncology_ext = self._load_oncology_extension()

        # Load OMOP vocabulary dictionary (pre-filtered oncology subset)
        self._dictionary = OMOPDictionary()
        self._dictionary.load()
        logger.info(
            "OMOP ontology initialized with %d vocabulary concepts",
            self._dictionary.concept_count,
        )
...
low-level clinical events to high-level disease and treatment abstractions."""
    ...
    @property
    def dictionary(self) -> OMOPDictionary:
        """Access the loaded OMOP vocabulary dictionary."""
        return self._dictionary

    def get_code_resolver(self):
        """Build a GenericCodeResolver from OMOP concept codes.

        Maps concept_code values from each vocabulary into the
        GenericCodeResolver so that LLM outputs referencing source codes
        (ICD-10, CPT, LOINC, RxNorm, etc.) can be resolved to standard
        OMOP concepts.
        """
        from ....extraction.code_resolver import GenericCodeResolver

        valid_values_map: dict[str, dict[str, str]] = {}

        # Build per-vocabulary code tables: {vocab_id: {concept_code: concept_name}}
        for vocab_id in self._dictionary.vocabularies:
            codes: dict[str, str] = {}
            for concept in self._dictionary.get_concepts_by_vocabulary(vocab_id):
                codes[concept.concept_code] = concept.concept_name
            if codes:
                valid_values_map[vocab_id] = codes

        # Also build per-domain code tables: {domain_id: {concept_code: concept_name}}
        for domain_id in self._dictionary.domains:
            codes = {}
            for concept in self._dictionary.get_concepts_by_domain(domain_id):
                codes[concept.concept_code] = concept.concept_name
            if codes:
                valid_values_map[domain_id] = codes

        return GenericCodeResolver(valid_values_map)
