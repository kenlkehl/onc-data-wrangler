...
Loads the authoritative BPC variable synopsis Excel dictionary and exposes a
GenericCodeResolver built from the dictionary's valid values.
"""
import json
import logging
...
from .dictionary import PRISSMMDictionary
...
class PRISSMMOntology(OntologyBase):
    ...
    def __init__(self):
        ...
        # Load the PRISSMM data dictionary from Excel
        self._dictionary = PRISSMMDictionary()
        self._dictionary.load()

        # Build a GenericCodeResolver from the dictionary's valid values
        valid_values_map = self._dictionary.get_valid_values_map()
        self._code_resolver = GenericCodeResolver(valid_values_map) if valid_values_map else None
        logger.info(
            "PRISSMM ontology initialised: dictionary=%s, code_resolver fields=%d",
            "loaded" if self._dictionary.loaded else "not loaded",
            len(valid_values_map),
        )
    ...
    @property
    def dictionary(self) -> PRISSMMDictionary:
        """The loaded PRISSMM data dictionary."""
    ...
    def get_code_resolver(self):
        """Return a GenericCodeResolver built from the PRISSMM dictionary's valid values."""
