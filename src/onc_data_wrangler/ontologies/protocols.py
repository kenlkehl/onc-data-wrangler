...
class DictionaryItemLike(Protocol):
    ...
    For NAACCR, this is satisfied by ``NAACCRDataItem`` with
    ``field_id = str(item_number)`` and the xml_id used as prompt field name.
    For other ontologies, it is satisfied by an adapter around ``DataItem``.
    """
...
class CodeResolverLike(Protocol):
    ...
    def get_valid_codes_prompt(self, field_id: str) -> str:
        ...
        Returns empty string if no code table exists for this field.
        """
        ...
...
class SchemaResolverLike(Protocol):
    ...
    def resolve_schema(self, context: dict[str, str]) -> str:
        ...
        Returns a schema identifier string (e.g. 'breast', 'lung', 'generic').
        """
        ...
