...
async def _run_setup_agent_async(
    ...
    Uses Claude Agent SDK or Ollama for bidirectional, interactive
    ...
    Args:
        ...
        model: Model name (required for ollama, optional for claude).
    ...
    logger.info("Starting setup agent (provider=%s, max_budget_usd=$%.2f)", provider, max_budget_usd)
    ...
    if last_result_message:
        ...
        if provider == "claude":
            ...
            logger.info(
                "Ollama session: %d turns, %d ms",
                getattr(last_result_message, "num_turns", 0),
                getattr(last_result_message, "duration_ms", 0),
            )
