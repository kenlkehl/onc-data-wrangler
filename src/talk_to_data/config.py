...
class VLLMServerConfig:
    """Configuration for auto-managed vLLM server subprocesses."""
...
class ExtractionConfig:
    ...
    vllm_servers: VLLMServerConfig = field(default_factory=VLLMServerConfig)
...
@dataclass
class ChatbotConfig:
    """Configuration for the web chatbot."""
    llm: LLMConfig = field(default_factory=lambda: LLMConfig(provider="anthropic", model="claude-sonnet-4-20250514"))
...
def load_config(path: str) -> ProjectConfig:
    ...
    if ext:
        ...
        vllm_servers_dict = ext.pop("vllm_servers", None)
        ...
        if vllm_servers_dict:
            known_vs = {f.name for f in VLLMServerConfig.__dataclass_fields__.values()}
            config.extraction.vllm_servers = VLLMServerConfig(**{k: v for k, v in vllm_servers_dict.items() if k in known_vs})
    ...
    # Chatbot
    chat = raw.get("chatbot", {})
    if chat:
        llm_dict = chat.pop("llm", None)
        known_chat = {f.name for f in ChatbotConfig.__dataclass_fields__.values()}
        config.chatbot = ChatbotConfig(**{k: v for k, v in chat.items() if k in known_chat})
        if llm_dict:
